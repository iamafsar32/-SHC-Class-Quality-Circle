...
<head>
    ...
    <title>Course History | Neural Portal</title>
...
<aside class="fixed left-0 top-0 h-[calc(100vh-2rem)] w-72 glass-panel z-50 hidden lg:flex flex-col p-8 m-4 rounded-[2.5rem]">
    <div class="mb-12 flex items-center gap-4">
        ...
        <div>
            <span class="text-xl font-black tracking-tighter block uppercase">ACADEMIC</span>
            <span class="text-[10px] font-bold opacity-40 uppercase tracking-[0.2em]">Course History</span>
        </div>
    </div>
    ...
    <nav class="space-y-2 flex-grow">
        ...
        <a href="course_history.php" class="flex items-center gap-3 px-5 py-4 rounded-2xl bg-primary/10 border-l-4 border-primary font-bold text-primary transition-all">
...
<main class="lg:ml-80 p-2 lg:p-10">
    <header class="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
        <div>
            <h1 class="text-5xl font-extrabold tracking-tighter uppercase">Academic <span class="text-gradient">Logs</span></h1>
            <p class="opacity-50 font-bold text-xs uppercase tracking-[0.3em] mt-2">Chronological Delivery stream</p>
        </div>
        
        <div class="flex items-center gap-4 glass-panel p-3 pr-8 rounded-2xl border-primary/20">
            <div class="w-12 h-12 rounded-xl action-gradient flex items-center justify-center text-white font-black">
                <i data-lucide="user-check" class="w-5 h-5"></i>
            </div>
            <div>
                <p class="text-[10px] font-bold opacity-40 uppercase tracking-widest">Active Faculty</p>
                <span class="text-sm font-black italic uppercase"><?php echo date('l, d M Y'); ?></span>
            </div>
        </div>
    </header>
    ...
    <div class="glass-panel rounded-[3rem] p-6 lg:p-10">
        <div class="flex items-center justify-between mb-10">
            <div class="flex items-center gap-6">
                ...
                <h3 class="text-2xl font-black tracking-tight uppercase">Teaching Records</h3>
            </div>
            ...
            <table class="w-full text-left border-separate border-spacing-y-3">
                <thead>
                    <tr class="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">
                        ...
                        <th class="px-6 py-4">Content Delivered</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $i = 1;
                    $q = mysqli_query($conn, "SELECT * FROM course_plan ORDER BY plan_date DESC");

                    if(mysqli_num_rows($q) > 0) {
                        while($r = mysqli_fetch_assoc($q)){
                            $raw_date = $r['plan_date'];
                            ?>
                            <tr class="glass-panel table-row-hover transition-all group">
                                <td class="px-6 py-5 first:rounded-l-2xl border-y border-l border-white/5">
                                    <span class="font-mono font-bold text-primary text-sm">#<?php echo str_pad($i, 2, '0', STR_PAD_LEFT); ?></span>
                                </td>

                                <td class="px-6 py-5 border-y border-white/5">
                                    <div class="flex flex-col">
                                        <div class="flex items-center gap-2 font-mono font-bold text-sm">
                                            <i data-lucide="calendar-days" class="w-3.5 h-3.5 text-primary"></i>
                                            <?php echo date("d M, Y", strtotime($raw_date)); ?>
                                        </div>
                                        <span class="text-[10px] opacity-40 uppercase font-black ml-5.5 mt-1">
                                            <?php echo date('l', strtotime($raw_date)); ?>
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-5 border-y border-white/5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="font-extrabold uppercase text-sm tracking-tight text-gradient">
                                ...
                                <td class="px-6 py-5 last:rounded-r-2xl border-y border-r border-white/5">
                                    <div class="max-w-md text-sm opacity-80 leading-relaxed italic border-l-2 border-white/10 pl-4">
                                        <?php echo nl2br(htmlspecialchars($r['course_content'])); ?>
                                    </div>
                                </td>
                            </tr>
                            <?php
    ...
    <footer class="mt-20 mb-10 text-center">
        <p class="text-[9px] font-black opacity-20 uppercase tracking-[1em]">
            &copy; <?php echo date('Y'); ?> FACULTY NODE • SECURE ACADEMIC HISTORY • BCA PROJECT
        </p>
    </footer>
</main>
