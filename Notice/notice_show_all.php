...
<head>
    ...
    <title>Notice History | Archive Node</title>
...
<aside class="fixed left-0 top-0 h-[calc(100vh-2rem)] w-72 glass-panel z-50 hidden lg:flex flex-col p-8 m-4 rounded-[2.5rem]">
    <div class="mb-12 flex items-center gap-4">
        ...
        <div>
            <span class="text-xl font-black tracking-tighter block uppercase">NOTICE NODE</span>
            <span class="text-[10px] font-bold opacity-40 uppercase tracking-[0.2em]">Archive Center</span>
        </div>
    </div>

    <nav class="space-y-2 flex-grow">
        <a href="notice_dashboard.php" class="flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-white/5 transition-all font-semibold opacity-70 hover:opacity-100">
...
<main class="lg:ml-80 p-2 lg:p-10">
    <header class="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
        <div>
            <h1 class="text-5xl font-extrabold tracking-tighter uppercase">Notice <span class="text-gradient">Archives</span></h1>
            <p class="opacity-50 font-bold text-xs uppercase tracking-[0.3em] mt-2">Historical Records Retrieval</p>
        </div>
        
        <div class="flex items-center gap-4 glass-panel p-3 pr-8 rounded-2xl border-primary/20">
            <div class="w-12 h-12 rounded-xl action-gradient flex items-center justify-center text-white font-black">N</div>
            <div>
                <p class="text-[10px] font-bold opacity-40 uppercase tracking-widest">Authorized Access</p>
    ...
    <div class="glass-panel rounded-[3rem] p-6 lg:p-10">
        <div class="flex items-center justify-between mb-10">
            <div class="flex items-center gap-6">
                ...
                <h3 class="text-2xl font-black tracking-tight uppercase">Stored Broadcasts</h3>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-separate border-spacing-y-3">
                ...
                <tbody>
                    ...
                    $sql = "SELECT n.*, s.name 
                            FROM notice n
                            LEFT JOIN students s ON UPPER(n.regno) = UPPER(s.reg_no) 
                            ...
                            <tr class="glass-panel table-row-hover transition-all group">
                                ...
                                <td class="px-6 py-5 border-y border-white/5">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-sm"><?= date('d M Y', strtotime($row['notice_date'])) ?></span>
                                        <span class="text-[10px] opacity-40 uppercase font-bold"><?= date('l', strtotime($row['notice_date'])) ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-5 border-y border-white/5">
                                    <div class="flex flex-col">
                                        <span class="text-primary font-black text-sm tracking-tight"><?= strtoupper($row['regno']) ?></span>
                                ...
                                <td class="px-6 py-5 last:rounded-r-2xl border-y border-r border-white/5">
                                    <p class="text-xs leading-relaxed opacity-70 group-hover:opacity-100 transition-opacity max-w-md">
                                        <?= nl2br(htmlspecialchars($row['notice_content'])) ?>
                                    </p>
                                </td>
                            </tr>
                            <?php
    ...
    <footer class="mt-20 mb-10 text-center">
        <p class="text-[9px] font-black opacity-20 uppercase tracking-[1em]">&copy; <?= date('Y') ?> NOTICE NODE • OFFICIAL ARCHIVE SYSTEM • BCA PROJECT</p>
    </footer>
</main>
